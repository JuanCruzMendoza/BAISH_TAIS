"""`pct_reads` vs layer, from csv/ into figures/. No recomputation.

    python plot_layer_curves.py <model> --tag 1K_per_direction

One figure per probe. `--per-family` probes get one curve per jailbreak family; every
other probe gets the `all` slice alone. The final layer is not drawn: it is read after the
final norm, so it is not the same object as the block outputs below it.

`--all-layers` reads the `jb_metrics__<rule>__all` run, so every layer is drawn and not
only the reporting band.
"""
import argparse
import csv
import sys
from pathlib import Path

import matplotlib

matplotlib.use("Agg")
import matplotlib.pyplot as plt

sys.path.insert(0, str(Path(__file__).resolve().parents[2]))

from experiments.common import config as cfg, manifest as mf

# Fixed so the two per-family figures are read against each other, and ordered
# most- to least-narrative rather than alphabetically.
FAMILIES = [("fiction_narrative", "Fiction / narrative", "#3b4cc0"),
            ("hybrid", "Hybrid", "#00929c"),
            ("roleplay_persona", "Roleplay / persona", "#e8871a"),
            ("nonfiction_other", "Nonfiction / other", "#9a9a9a")]

PROBE_NAMES = {"story_v2_1k": "Story", "persona_v2": "Persona",
               "harm_v2": "Harm", "eval_v2": "Eval"}


def probe_name(probe):
    return PROBE_NAMES.get(probe, probe.split("_v")[0].replace("_", " ").capitalize())


def read_rows(path):
    with Path(path).open(encoding="utf-8-sig", newline="") as f:
        return list(csv.DictReader(f))


def series(rows, probe, kind, group, drop=None):
    """Layers with ref_tpr == 0 are dropped: tau sits above the whole positive pole
    there, so nothing can clear it and pct_reads = 0 is the bar failing rather than a
    reading. Only L0, the embedding output, does this. `drop` takes out the model's final
    layer, which is read after the final norm and is not comparable with the block
    outputs below it -- it is only set when the run swept every layer."""
    xy = sorted((int(r["layer"]), float(r["pct_reads"]))
                for r in rows if r["probe"] == probe
                and r["group_kind"] == kind and r["group"] == group
                and float(r["ref_tpr"]) > 0 and int(r["layer"]) != drop)
    return [p[0] for p in xy], [p[1] for p in xy]


def curves(rows, probe, per_family, drop=None):
    if not per_family:
        x, y = series(rows, probe, "all", "all", drop)
        return [(x, y, "All families", "#3b4cc0", 2.0)]
    out = []
    for fam, label, colour in FAMILIES:
        x, y = series(rows, probe, "family", fam, drop)
        if x:
            out.append((x, y, label, colour, 1.7))
    return out


def plot(rows, probe, layer, per_family, rule, band, path, drop=None):
    cs = curves(rows, probe, per_family, drop)
    fig, ax = plt.subplots(figsize=(7.2, 4.4))
    for x, y, label, colour, lw in cs:
        ax.plot(x, y, marker="o", ms=3.2, lw=lw, color=colour, label=label)
    # A gap in x means a layer outside the band was scored because it was chosen.
    if layer is not None:
        ax.axvline(layer, color="0.2", lw=1.0, ls=":", zorder=0)
        # Inside the axes and rotated: at the band edge a centred label above the axes
        # runs into the title.
        ax.annotate(f" chosen L{layer}", (layer, 99), fontsize=8, color="0.25",
                    ha="left", va="top", rotation=90)
    ax.set_xlabel("layer")
    ax.set_ylabel(f"% of jailbreaks clearing $\\tau$  ({rule})")
    ax.set_title(f"{probe_name(probe)} — Reads of jailbreaks vs layer", fontsize=11)
    ax.set_ylim(-3, 103)
    ax.set_xticks([l for l in cs[0][0] if l % 2 == int(band[0]) % 2] or cs[0][0])
    ax.grid(alpha=0.25, lw=0.5)
    ax.legend(fontsize=8, loc="best", framealpha=0.9)
    fig.tight_layout()
    fig.savefig(path, dpi=160)
    plt.close(fig)


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("model")
    ap.add_argument("--tag", default=None)
    ap.add_argument("--threshold", default="midpoint")
    ap.add_argument("--per-family", default="story_v2_1k,persona_v2",
                    help="probes drawn as one curve per family; the rest get `all`")
    ap.add_argument("--all-layers", action="store_true",
                    help="read the __all metrics run (every layer, not only the band)")
    args = ap.parse_args()

    lay = cfg.Layout("probe_jailbreak_detection", args.model, args.tag, acts_cache=False)
    src = mf.stem("jb_metrics", args.threshold, "all" if args.all_layers else None)
    up = mf.load_upstream(lay.meta / f"{src}_manifest.json")
    rows = read_rows(lay.csv / f"{src}_rate.csv")

    probes = up["config"]["probes"]
    chosen = {a: int(l) for a, l in (up["config"].get("probe_layers") or {}).items()}
    band = up["config"]["band"]
    # Only an all-layer sweep reaches the final layer, so only there is the max a layer
    # to drop; in a band run it is just the top of the band.
    drop = (max(up["config"]["layers"])
            if up["config"].get("layer_scope") == "all" else None)
    fam = [p for p in args.per_family.split(",") if p]
    unknown = [p for p in fam if p not in probes]
    if unknown:
        raise SystemExit(f"--per-family: not in the run: {unknown}; it holds {probes}")

    stem = mf.stem("plot_layer_curves", "all" if args.all_layers else None)
    config = {"probes": probes, "threshold_rule": args.threshold, "per_family": fam,
              "chosen_layers": chosen or None,
              "layer_scope": up["config"].get("layer_scope", "band")}
    inputs = {"jb_metrics_run_key": up["run_key"]}

    with mf.Run(lay, stem, config, inputs) as run:
        for p in probes:
            path = run.artefact(f"_{p}.png")
            plot(rows, p, chosen.get(p), p in fam, args.threshold, band, path, drop)
            print(f"  {Path(path).relative_to(lay.root).as_posix()}"
                  f"   {'4 families' if p in fam else 'all families'}")


if __name__ == "__main__":
    main()
