...
def _():
    ...
    # breaks every target-vs-noop comparison at this tag, silently. It can never be
    # *lowered* afterwards either, so it is sized to fit rather than to be fast.
    #
    # One 96 GB Blackwell: 32.8B in bf16 is 65.6 GB of weights, and this model's KV is
    # 256 KiB/token (2 x 64 layers x 8 KV heads x 128 dims, fp16) -- 4.6x the 7B's per
    # token, which is why Qwen2.5-7B's 32/65536 is impossible here. `max_batch_tokens`
    # bounds len(batch) x longest_prompt, so 32768 lets a batch reach 32 only when its
    # longest prompt is <= 1024; the jailbreak p95 is 1,063 tokens and the max is 8,688, and
    # those long ones fall into batches of 3 on their own. Worst case is
    # 32 x (1024 + 512) x 256 KiB = 12 GiB of KV over the weights -- ~81 GB of 96.
    BATCH = ("--batch-size", "32", "--max-batch-tokens", "32768")
    ...
    # >= 4.51 rather than gemma's 4.44: `logits_to_keep` has to reach the Qwen2 forward, or
    # prefill materialises [batch, seq, 152k] logits -- 10 GB at this batch, for one row of
    # which is used. No `flash-attn`: its prebuilt wheels have no sm_120 kernel, and sdpa is
    # torch-native and correct for this architecture. `openai` is the judge's SDK, and
    # OpenRouter speaks the same protocol so the fallback needs no second package.
    sh("pip", "install", "-q", "transformers>=4.51", "accelerate", "numpy", "matplotlib",
       "huggingface_hub", "hf_xet", "openai")
    # Fails here rather than 20 min into a model load: Blackwell is sm_120 and a wheel built
    # before CUDA 12.8 has no kernel image for it, which surfaces as a cryptic launch error
    # deep inside the first forward.
    sh("python", "-c",
       "import torch;cc=torch.cuda.get_device_capability(0);p=torch.cuda.get_device_properties(0);"
       "print(torch.__version__,'cuda',torch.version.cuda,p.name,'sm%d%d'%cc,"
       "'%.0f GiB'%(p.total_memory/2**30));"
       "assert cc<(12,0) or tuple(map(int,torch.__version__.split('.')[:2]))>=(2,7),"
       "'sm%d%d (Blackwell) needs torch>=2.7 on CUDA 12.8; this wheel has no kernel for it'%cc")
...
def _(MODEL, TAG, mo):
    ...
    Spec: `research/spec-whole-rerun.md`. The GPU half of the pipeline for the third model:
    ...
    **Same tag, different model slug**, so nothing here can collide with the Qwen 7B or the
    gemma run. Dropped from the 7B run: `ablate`, `cap`, the `length` foil, the `random`
    arm, the decoding comparison (greedy, reused) and the second-layer pass.
    **α = 0.25 / 0.50 / 0.75 / 1.00** for every direction, sign derived from
    `cell.RESTORE_SIGN`.
    ...
    **This notebook does what needs the GPU, plus whatever is free because the data is
    ...
    - **cross_probe_detection (§3) runs here only if the pole cache is already on this box**
      — i.e. extraction ran in *this* session. It is CPU-only, so when the 4.8 GiB is on
      local disk it is ~2 min and free; on a fresh instance that resumed from complete
      manifests it is skipped rather than pulling that cache back, since that download is
      what kept hitting the Xet read-token 429 at a third of the size.

    Five points stop and wait for you:

    | stop | what to read first | what to do |
    |---|---|---|
    | **gate 1 — layers** | `probe_select__<axis>.csv` (`cohens_dz_train`) and, for story, the fiction − nonfiction `pct_reads` gap | type one layer per direction, two where the criteria disagree |
    ...
    ## Qwen2.5-32B specifics

    - **L = 64, d = 5120**, so every absolute layer is re-derived here; none of the 7B's or
      gemma's carry over. Gate 1 below is **not** prefilled for that reason.
    - **No `ATTN_IMPL`**, deliberately — unlike gemma-2, this architecture has nothing sdpa
      drops, and sdpa is the kernel that exists on sm_120.
    - **Not a gated repo**: the HF token below is only for checkpointing, not for the weights.
    - **`$BLOB_STORE` must stay unset.** Blobs are keyed by prompt token ids and this shares
      a tokenizer with Qwen2.5-7B, so a store pointed at the 7B run would silently serve its
      activations for every prompt. Unset means per-model, which is what is wanted.
    - **The pole cache is 4.8 GiB here** (7,731 blobs × 65 × 5120 fp16) against gemma's 2.23,
      on the same 7,731 files — only the bytes scale, and the file count is what 429s.
    """)
    return
...
def _(mo):
    # cell 3
    # The only credential this notebook needs -- and only to checkpoint: Qwen2.5-32B is not
    # gated. No judge key: grading runs off the GPU box (see the hand-off cells), so an API
    # key never reaches the rented instance.
    HF_TOKEN = mo.ui.text(label="HF_TOKEN (write access, for checkpointing)",
...
def _(mo):
    ...
    **The extraction scope is never pushed after the jailbreak activations are cached.**
    ...
    the 4.8 GiB cache for activations nothing downstream reads.
...
def _(HF_REPO, HF_TOKEN, MODEL, REPO, TAG, mo, os):
    ...
    mo.stop(not HF_TOKEN.value,
            mo.md("*Paste the HF token to start. It checkpoints every result; nothing "
                  "runs without it.*"))
    # from_pretrained in every subprocess authenticates with $HF_TOKEN -- the same token
    # that checkpoints the results. The weights themselves are ungated here.
    os.environ["HF_TOKEN"] = HF_TOKEN.value
    # No ATTN_IMPL: gemma-2 needed `eager` because sdpa drops its attention soft-cap, and
    # this architecture has no such thing. Unset leaves transformers' own default (sdpa),
    # which is also the only one with an sm_120 kernel.
    os.environ.pop("ATTN_IMPL", None)
    # Keyed by prompt token ids, and Qwen2.5-7B shares this tokenizer -- a shared store
    # would hand back the 7B's activations for every prompt without a word.
    if os.environ.get("BLOB_STORE"):
        raise RuntimeError("unset $BLOB_STORE: it is shared by token ids and Qwen2.5-7B "
                           "has the same tokenizer, so the 7B's blobs would be served here")
...
def _(AXES, CP_SCOPE, DONE, EX_ROOT, EX_SCOPE, HF_REPO, HF_TOKEN, JB_ROOT, JB_SCOPE,
    ...
    # done the 4.8 GiB never comes down again -- which is what removes the Xet read-token 429
    ...
    if not (EX_CACHED and DONE(JB_ROOT, "jb_readout")):
        ...
        print("extraction acts: nothing on this box requires them -- skipped (4.8 GiB)")
    pulled = True
...
def _(AXES, EX_CACHED, EX_SCOPE, HF_REPO, HF_TOKEN, MODEL, TAG, ckpt, pulled, sh):
    ...
    if EX_CACHED:
        ...
        # finished pass still costs 65.6 GB of weights. This is the skip that matters.
        ...
        try:
            ...
            _timer.set()
        # pack=True is what puts the cache on the Hub at all: ckpt.push holds loose blobs
        # back unconditionally, so blobs.tar is the only stored form. That guard used to
        # live under `if pack:`, which left the *other* pushes of this same scope (vectors,
        # figures -- both unpacked) free to upload all 7,731 .npy beside the tar.
...
def _(mo):
    ...
    `jb_readout.py`, and once its `.pt` is on the Hub they are disposable. Once that `.pt`
    exists the cell drops to `--view-only` — the view is a tokenizer artefact, seconds on
    CPU, and it is what `sets.py` rebuilds the steering prompt sets from.
    """)
    return
...
@app.cell(hide_code=True)
def _(BAND, N_LAYERS, TAG, mo):
    # cell 15 (markdown)
    mo.md(f"""
    ## ▸ GATE 1 — the chosen layers

    **Not prefilled: L = {N_LAYERS} here, so no layer from the 7B (L=28) or gemma (L=42)
    transfers.** Read them off the two tables this run just wrote:
    ...
    - `extraction/results/{TAG}/<model_slug>/csv/probe_select__<axis>.csv` —
      **`cohens_dz_train`**, confirmed against `cohens_dz_heldout` (200 pairs),
      `mean_paired_cos` and `lopo_ci_lo`. This is the default criterion: max
      `cohens_dz_train` with the smallest train↔held-out gap.
    ...
    **Give an axis two layers (`axis=L1+L2`) wherever those two criteria disagree.** On both
    earlier models they did, for story and only for story — 13 layers apart on gemma — and
    with one layer a null steering cell cannot be told apart from a wrong layer. It is also
    the only thing that produced a replicated result: L15 beat L28 on gemma and L11 beat L19
    on the 7B, i.e. `cohens_dz` picked the worse steering layer both times. Two layers cost
    4 extra cells per set; do not drop them to save that.

    The reporting band is **L{BAND[0]}–L{BAND[-1]}** (round(0.40·{N_LAYERS}) to
    round(0.90·{N_LAYERS})). A layer outside it is allowed but must be opted into: every
    cell that uses one carries `--allow-out-of-band`, added automatically below and recorded
    in its manifest.

    One caveat on the `_chosen` tables: **`jb_metrics` takes one layer per probe**, so it
    runs at each axis's **first** layer; a second layer's row is in the per-layer file the
    same run writes (`jb_metrics__<rule>__all_rate.csv`). `cross_auroc` and `geometry` do
    **not** have that limit — `a=L1+L2` gives them a second probe row, and they get both.

    Record the choice in `extraction/insights.md` once made: experiments 3–5 read it from
    there, never from a JSON.
    """)
    return
...
def _(mo):
    # cell 16
    # Deliberately blank -- unlike the gemma notebook, this gate is open: L=64 and none of
    # the earlier models' layers mean anything here. `+` gives an axis two layers.
    CHOSEN_IN = mo.ui.text(
        label="chosen layers (axis=layer, `+` for a second layer, comma-separated)",
        full_width=True, value="",
        placeholder="story_v2_1k=<l>+<l>,persona_v2=<l>,harm_v2=<l>,eval_v2=<l>")
    CHOSEN_IN
    return (CHOSEN_IN,)
...
def _(AXES, BAND, CHOSEN_IN, N_LAYERS, jb_all, mo):
    ...
    # Layers, not axes: a no-op is per (set, layer), and two axes at one layer share it.
    ...
    # axis -- the same vector read at another layer -- rather than a second axis. An axis
    # with two steering layers gets both, which is what tells a null cell from a wrong layer.
    ...
    print(f"depth fraction: " + ", ".join(a + " " + "/".join(f"{l / N_LAYERS:.2f}"
                                                             for l in ls)
                                          for a, ls in CHOSEN.items()))
...
def _(HF_REPO, MODEL, PROBE_ARG, TAG, jb_chosen, mo):
    ...
    - **it is not** — a fresh instance that resumed from complete extraction manifests never
      ...
      rather than pull 4.8 GiB whose per-chunk read tokens are what hit the Xet 429. Run it on
    ...
    **Host RAM, not VRAM, is the constraint in the on-box branch.** Axes load one at a time
    at 1,000 pairs × 65 layers × 5120 dims fp32 ≈ **2.7 GB per axis** — 3.3× what it was at
    the 7B, and no longer free beside a resident 65.6 GB model. If the instance is thin on
    system memory, take the local branch even when the cache is here.

    Either way `--layers` is `{PROBE_ARG}`: a `+` gives that axis a **second probe row**, the
    same vector read at another layer, not a second axis. Gate 3 reads the anchor-vs-partner
    cosine from `geometry_cos.csv`, which spans every band layer regardless.
...
def _(AXES, CHOSEN, CP_ROOT, CP_SCOPE, DONE, EX_ROOT, HF_REPO, HF_TOKEN, MODEL, PROBE_ARG,
    ...
    # raises on and `blobs/` is the 4.8 GiB itself, so both have to be there -- a tree pulled
    ...
    if _current:
        ...
        print("cross-probe: no pole cache on this box, and it is not worth 4.8 GiB to make "
...
def _(mo):
    ...
    That is **(axis, layer) pairs × 4 α** target cells per set, plus one no-op per
    (set, layer) — two axes sharing a layer share theirs. With one axis at two layers, as on
    both earlier models, it comes to 24 cells per set and **48 in total**, ≈22.6k
    generations. The cell below prints the actual count once gate 1 is answered.

    **Judging does not happen here.** It is API-bound, ~23k calls over 3–5 h, and a rented
...
def _(ALPHAS, CHOSEN, MODEL, N_LAYERS, OOB, ST_COMPLETE, TAG, mo, st_split):
    ...
    def _jobs(prompt_set):
        ...
        No `ablate`: it did not help in any config at the Qwen 7B run, so suppression is
        `add` at -alpha. The sign is derived, never typed.
        """
        out, flip = [], 1.0 if prompt_set == "success" else -1.0
        for axis, layers in CHOSEN.items():
            for layer in layers:            # an axis with two chosen layers runs at both
                for a in ALPHAS:
        ...
        # prompt_set x layers_spec, so two axes chosen at one layer share that no-op.
    ...
    for _set, _script in (("success", "steer_single"), ("refusal", "steer_induce")):
        ...
        _p = _P(_tf.gettempdir()) / f"qwen32b_jobs_{_set}.json"
...
def _(BATCH, HF_REPO, HF_TOKEN, MODEL, ST_JOBS, ST_SCOPE, TAG, ckpt, sh):
    ...
    # times -- 65.6 GB a load, so that matters more here than it did at 7B -- and only the
    # *pending* jobs are handed to it: a set with nothing left skips the process entirely
    # and pays no load. Resume is also per cell and per batch inside steer_batch, so a kill
...
def _(HF_REPO, HF_TOKEN, MODEL, ST_ALL_STEMS, ST_ROOT, ST_SCOPE, TAG, ckpt, mo, st_steered):
    ...
    **{len(_todo)} are not**. {'Everything is on the Hub.' if _ok is not None else
    ...
    scored, so re-running the loop after an interruption costs only what is left. Run **one**
    of these loops at a time: the append lock is per process, and two of them on the same
    `_judged.jsonl` interleave rows. Then read `aggregate_controls.csv` and come back for
    gate 3 — the pairs need the α curve.
...
def _(TAG, mo):
    ...
    - `cos(û_a, û_b)` at a's chosen layer clears the ±3/√d null band —
      ...
      At d = 5120 that band is ±0.042, tighter than either earlier model's ±0.050;
    ...
    *necessity* — and **`par_component`** — `c·û_b`, norm `|c|`, asks *sufficiency*.
    ...
    An anchor with two chosen layers must name one with `a@L`; without it the anchor's
    primary layer is used, and `b` is always read at the same layer as `a`:

    ```
    story_v2_1k@<l>>persona_v2 success=0.75 refusal=0.25
    persona_v2>eval_v2 success=0.5
    ```

    Expect ≤2 pairs. On gemma there was arguably **none**: every off-diagonal cosine came in
    at |cos| ≤ 0.17 and the two the 7B flagged (`persona`–`eval` +0.296, `harm`–`persona`
    −0.240) did not replicate. Inside the null band `perp` is the same experiment as
    `unprojected` and the script says so — leave the box blank rather than run it anyway.
    """)
    return
...
def _(mo):
    ...
    PAIRS_IN = mo.ui.text_area(
        ...
        placeholder="story_v2_1k@<l>>persona_v2 success=0.75 refusal=0.25")
    PAIRS_IN
    return (PAIRS_IN,)
...
def _(ALPHAS, AXES, CHOSEN, PAIRS_IN, ST_COMPLETE, mo, st_steered):
    ...
    def _parse(line):
        ...
        # `a@L` picks which of the anchor's chosen layers to project at -- an axis can have
        # two, and the projection is same-layer, so this is not a free parameter.
...
def _(BATCH, HF_REPO, HF_TOKEN, MODEL, OOB, PAIR_JOBS, ST_SCOPE, TAG, ckpt, sh):
    ...
    # ~3 min at 65.6 GB against ~20 min of generation per pair. Pairs whose arms are both
    # complete are dropped before that, so a resumed session pays only for what is left.
    # Inside an invocation the usual guarantees hold: resume per row, and a complete arm
    # cache-hits.
